...
if [[ "$MAP_NAME" = "greater-london" ]]; then
    ...
    curl -L https://download.geofabrik.de/europe/united-kingdom/england/greater-london-latest.osm.pbf -o greater-london.osm.pbf
elif [[ "$MAP_NAME" = "england" ]]; then
    echo "Downloading $MAP_NAME..."
    curl -L https://download.geofabrik.de/europe/united-kingdom/england-latest.osm.pbf -o england.osm.pbf
else
    echo "Unknown map name '$MAP_NAME'. Try 'greater-london' or 'england'."
    exit 1
fi
